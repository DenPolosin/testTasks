			...
			<div class="row">
				...
				<div class="<?=($showBlockWithOffersAndProps ? "col-lg-7" : "col-lg"); ?>">
					<div class="product-item-detail-pay-block">
						...
						{
                            $compareListIds = [];
						    if (!empty($_SESSION['CATALOG_COMPARE_LIST'][$arResult['IBLOCK_ID']]['ITEMS'])) {
                                $compareListIds = array_map(
                                    function ($item) {
                                        return $item['ID'];
                                    },
                                    $_SESSION['CATALOG_COMPARE_LIST'][$arResult['IBLOCK_ID']]['ITEMS']
                                );
                            }
							?>
                            <div class="mb-3" id="<?=$itemIds['COMPARE_LINK']?>">
                                <a class="btn btn-primary product-item-detail-buy-button" href="javascript:void(0)" rel="nofollow">
                                    <?= in_array($arResult['ID'], $compareListIds) ? $arParams['MESS_BTN_COMPARE_ALREADY'] : $arParams['MESS_BTN_COMPARE']?>
                                </a>
                            </div>
							<?
						}
